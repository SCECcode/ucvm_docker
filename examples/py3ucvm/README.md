...
## build an image
    ...
    docker image build -t py3ucvm .
    docker tag py3ucvm mpihuisu/py3ucvm:1.0
    docker push mpihuisu/py3ucvm:1.0

## running container from an image with a local host 'target' directory: 

    mkdir target
    docker run --rm -it --mount type=bind,source="$(pwd)"/target,destination=/app/target  mpihuisu/py3ucvm:1.0
...
## cleanup images
    ...
    docker rmi py3ucvm 
    docker rmi mpihuisu/py3ucvm:1.0 -f 
